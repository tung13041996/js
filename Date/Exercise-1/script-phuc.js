$(document).ready(function () {
    ...
    // stringIsDate("dd/mm/yyyy"){}
    function stringIsDate(string) {
        // length = 10
        // dd
        // mm
        // yyyy
    ...
    let dayOfWeek = function (d, m, y) {
        ...
        return day[(d + 2 * m + Math.floor((3 * (m + 1)) / 5) + y + Math.floor(y / 4)) % 7];
    }
    ...
    function isDateBiggerThanDate(string1, string2) {
        let date1 = stringIsDate(string1),
            date2 = stringIsDate(string2);

        return true;
    }

    //find zodiac "dd/mm/yyyy"
    let findZodiac = function (string) {
        ...
        let date = '';
        for (let i in zodiac) {
            if (date >= zodiac[i].from_date && date <= zodiac[i].from_date) {
                return 'name';
            }
        }
    ...
    let dateOfMonth = function (d, m, y) {
        let day_of_month = [31, checkLeapYear(y) ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
        return day_of_month[m - 1];
    }

    //find date of year 14/02/2021
